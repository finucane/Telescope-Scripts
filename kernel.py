...
for example
kernel.py --outdir=../scratch --indir=../campanas-all --vars t e --date=5-23-2008
""")
...
#get fps stuff. if fps is a fraction, convert to integer + repeat
assert args.fps > 0
if args.fps < 1:
  fpsRepeat = math.ceil (1.0/args.fps)
  fps = 1
else:
  fps = math.ceil (args.fps)
  fpsRepeat = 1
...
def draw_frame (fig, im, lines, day0, day1, xmin, xmax, ymin, ymax):
  ...
  #sometimes there's no data or no matching data
  assert len (measurements [0]) == len (measurements [1])
  ...
  #sometimes there's arrays that make singular matrixes, an example of this is
  #2 lists where each list has all the same values. i guess.
  try:
...
if args.date:
  ...
  pdf = PdfPages (os.path.join (args.outdir, "kernel_{}_{}_{}.pdf".format (
  ...
  os.path.basename (args.indir),
  startDate.strftime ("%m/%d/%Y"),
  stopDate.strftime ("%m/%d/%Y")))

  assert args.start_date and args.stop_date
  assert len (matchedDays [0])
  FFMpegWriter = manimation.writers['ffmpeg']
  writer = FFMpegWriter (fps=fps, bitrate=args.bitrate)
  with writer.saving (fig, os.path.join (args.outdir, "kernel_{}_{}_{}_{}.mp4".format (
    ...
    for day0, day1, dayCount in zip (matchedDays [0], matchedDays [1], range (len (matchedDays [0]))):
      if draw_frame (fig, im, lines, day0, day1, xmin, xmax, ymin, ymax):
        date = day0.datetime.date ()
        ax.set_title ("Kernel Density Estimation \n {} - {}".format (os.path.basename (args.indir), date.strftime ("%m/%d/%Y")))
        for _ in range (fpsRepeat):
          writer.grab_frame ()
